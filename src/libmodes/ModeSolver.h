...
namespace NCPA {
    class ModeSolver : public AtmosphericTransferFunctionSolver {
        public:
            ...
            // parameter control
            virtual void setParams( ParameterSet *param,
                                    Atmosphere1D *atm_prof )
                = 0;
            virtual void printParams() = 0;

            // general functions
            int getNumberOfModes( int n, double dz, double *diag, double k_min,
                                  double k_max, int *nev );
            int sturmCount( int n, double dz, double *diag, double k,
                            int *cnt );
            int doPerturb( int nz, double z_min, double dz, int n_modes,
                           double freq, double *k, double **v, double *alpha,
                           std::complex<double> *k_pert );
            int getTLoss1D( int select_modes, double dz, int n_r, double dr,
                            double z_src, double z_rcv, double *rho,
                            std::complex<double> *k_pert, double **v_s,
                            std::string filename_lossy,
                            std::string filename_lossless = "" );
            int getTLoss1DNx2( double azimuth, int select_modes, double dz,
                               int n_r, double dr, double z_src, double z_rcv,
                               double *rho, std::complex<double> *k_pert,
                               double **v_s, bool Nx2, int iter,
                               std::string filename_lossy,
                               std::string filename_lossless = "" );
            int getTLoss2D( int nz, int select_modes, double dz, int n_r,
                            double dr, double z_src, double *rho,
                            std::complex<double> *k_pert, double **v_s,
                            std::string filename_lossy,
                            std::string filename_lossless = "" );
            int writeDispersion( int select_modes, double dz, double z_src,
                                 double z_rcv, double freq,
                                 std::complex<double> *k_pert, double **v_s,
                                 double *rho );
            int writeDispersion( FILE *fp, int select_modes, double dz,
                                 double z_src, double z_rcv, double freq,
                                 std::complex<double> *k_pert, double **v_s,
                                 double *rho );
            ...
            // to be overridden
            virtual int doSelect( int nz, int n_modes, double k_min,
