...
void NCPA::ESSModeSolver::setParams( NCPA::ParameterSet *param,
    ...
    // if (write_phase_speeds || write_speeds || write_2D_TLoss || write_modes
    ...
    Naz = 1;  // Number of azimuths: default is a propagation along a single
              // azimuth
    if (Nby2Dprop) {
    ...
    // Note units: height in m, wind speeds in m/s, pressure in Pa(?), density
    // in kg/m3
    ...
    c_eff = new double[ Nz_grid ];  // to be filled in getModalTrace(), depends
                                    // on azimuth
    alpha = new double[ Nz_grid ];


    //
    // !!! ensure maxheight is less than the max height covered by the provided
    // atm profile may avoid some errors asociated with the code thinking that
    // it goes above the max height when in fact the height may only differ
    // from max height by a rounding error. This should be revisited.
    // @todo revisit this
    ...
    if (atm_profile->contains_vector( "C0" )) {
        ...
        if (atm_profile->contains_vector( "P" )
            ...
            std::cout << "Calculating sound speed from pressure and density."
                      << std::endl;
            ...
            throw std::runtime_error(
                "Can't calculate static sound speed, either temperature T or "
    ...
    if (atm_profile->contains_vector( "RHO" )) {
        atm_profile->convert_property_units( "RHO",
                                             Units::fromString( "kg/m3" ) );
    } else {
        throw std::runtime_error(
    ...
    if (atm_profile->contains_scalar( "Z0" )) {
        ...
        if (z_min < profile_z0) {
            std::cout
                << "Adjusting minimum altitude to profile ground height of "
    ...
    for (fi = 0; fi < Nfreq; fi++) {
        ...
        for (it = 0; it < Naz; it++) {
            ...
            if (gnd_imp_model.compare( "rigid" ) == 0) {
                ...
                if (Lamb_wave_BC) {
                    admittance
                        = -atm_profile->get_first_derivative( "RHO", z_min )
            ...
            //
            getModalTrace( Nz_grid, z_min, sourceheight, receiverheight, dz,
                           atm_profile, admittance, freq, azi, diag, &k_min,
                           &k_max, use_WKB, c_eff );
            ...
            printf( "_________________________________________________________"
                    "_____________\n\n" );
            printf( " -> Normal mode solution at %5.3f Hz and %5.2f deg (%d "
                    "modes)...\n",
                    freq, azi, nev );
            ...
            doSelect( Nz_grid, nconv, k_min, k_max, k2, v, k_s, v_s,
                      &select_modes );
            doPerturb( Nz_grid, z_min, dz, select_modes, freq, k_s, v_s, alpha,
                       k_pert );
            ...
            if (Nby2Dprop) {  // if (N by 2D is requested)
                fname_lossy
                    = tag_filename( NCPAPROP_MODESS_FILENAME_1D_MULTIPROP );
                fname_lossless = "";
                if (write_lossless) {
                ...
                getTLoss1DNx2( azi, select_modes, dz, Nrng_steps, rng_step,
                               ...
                               Nby2Dprop, it, fname_lossy, fname_lossless );
            } else {
                ...
                if (write_lossless) {
                    fname_lossless
                        = tag_filename( NCPAPROP_MODESS_FILENAME_1D_LOSSLESS );
                ...
                if (!modstartfile.empty()) {
                    ...
                    // Modification to apply the sqrt(k0) factor to agree with
                    // Jelle's getModalStarter; this in turn will make 'pape'
                    // agree with modess output
                    getModalStarter( Nz_grid, select_modes, dz, freq,
                                     sourceheight, receiverheight, rho, k_pert,
                                     v_s, tag_filename( modstartfile ) );
                }
                ...
                if (write_modes) {
                    ...
                    writePhaseAndGroupSpeeds(
                        Nz_grid, dz, select_modes, freq, k_pert, v_s, c_eff,
                        tag_filename(
                            NCPAPROP_MODESS_FILENAME_PHASE_AND_GROUP_SPEEDS ) );
                }

                if (( write_speeds ) && !( write_modes )) {
                    cout << "Writing to file: the modal phase speeds and the "
                            "group speeds...\n";
                    writePhaseAndGroupSpeeds(
                        Nz_grid, dz, select_modes, freq, k_pert, v_s, c_eff,
                        tag_filename(
                ...
                if (!dispersion_file.empty()) {
                    printf( "Writing to file %s: dispersion at freq = %8.3f "
                            "Hz...\n",
            ...
            // Clean up azimuth-specific atmospheric properties before starting
            // next run
...
int NCPA::ESSModeSolver::getModalTrace(
    ...
    double *diag, double *k_min, double *k_max, bool use_WKB, double *ceffz ) {
    ...
    // NCPA::UNITS_ANGLE_DEGREES, UNITS_ANGLE_RADIANS ); double gamma = 1.4;
    ...
    *k_min    = omega / ceffmax;
    for (i = 0; i < nz; i++) {
        ...
        if (ceffz[ i ] < ceffmin) {
            ceffmin = ceffz[ i ];  // approximation to find minimum sound speed
                                   // in problem
        }
        if (ceffz[ i ] > ceffmax) {
            ceffmax = ceffz[ i ];
...
void NCPA::ESSModeSolver::getModalStarter( int nz, int select_modes, double dz,
                                           double freq, double z_src,
                                           double z_rcv, double *rho,
                                           complex<double> *k_pert,
                                           double **v_s,
...
int NCPA::ESSModeSolver::writePhaseAndGroupSpeeds(
    ...
    for (j = 0; j < select_modes; j++) {
        ...
        for (n = 0; n < nz; n++) {
            v_group
                = v_group
                + v_s[ n ][ j ] * v_s[ n ][ j ] / ( c_eff[ n ] * c_eff[ n ] );
        }
