...
namespace NCPA {
    ...
    NCPA::ESSModeSolver::ESSModeSolver( NCPA::ParameterSet *param,
    ...
    void NCPA::ESSModeSolver::setParams( NCPA::ParameterSet *param,
                                         NCPA::Atmosphere1D *atm_prof ) {
        ...
        this->write_2D_TLoss = param->getBool( "write_2d_tloss" );
        ...
        // this->use_WKB          = !param->getBool( "turnoff_WKB" );
        this->use_WKB          = param->getBool( "use_WKB" );
        ...
        // write_modes
        //     || ( dispersion_file.size() > 0 )) {
        //     use_WKB = false;  // don't use WKB least phase speed estim. when
        //     saving
        //                       // any of the above values
        // }

        Naz = 1;  // Number of azimuths: default is a propagation along a
                  // single azimuth
        if (Nby2Dprop) {
        ...
        // get Hgt, zw, mw, T, rho, Pr in SI units; they are freed in
        // computeModes()
        // @todo write functions to allocate and free these, it shouldn't be
        // hidden Note units: height in m, wind speeds in m/s, pressure in
        // Pa(?), density in kg/m3
        ...
        // provided atm profile may avoid some errors asociated with the code
        // thinking that it goes above the max height when in fact the height
        // may only differ from max height by a rounding error. This should be
        // revisited.
        // @todo revisit this
        ...
        double dz
            = ( maxheight - z_min ) / ( Nz_grid - 1 );  // the z-grid spacing
        ...
        if (atm_profile->contains_vector( "C0" )) {
            ...
            atm_profile->convert_property_units( "_C0_",
                                                 Units::fromString( "m/s" ) );

            // do we need to calculate temperature?
            if (!atm_profile->contains_vector( "T" )) {
                std::cout
                    << "No temperature provided, calculating from sound speed"
                    << std::endl;
                atm_profile->calculate_temperature_from_sound_speed(
                    "T", "_C0_", Units::fromString( "K" ) );
            }
        } else {
            if (atm_profile->contains_vector( "P" )
                && atm_profile->contains_vector( "RHO" )) {
                std::cout
                    << "Calculating sound speed from pressure and density."
                    << std::endl;
                atm_profile->calculate_sound_speed_from_pressure_and_density(
                    "_C0_", "P", "RHO", Units::fromString( "m/s" ) );
            } else if (atm_profile->contains_vector( "T" )) {
                std::cout << "Calculating sound speed from temperature"
                          << std::endl;
                atm_profile->calculate_sound_speed_from_temperature(
                    "_C0_", "T", Units::fromString( "m/s" ) );
            } else {
                throw std::runtime_error(
                    "Can't calculate static sound speed, either temperature T "
                    "or "
                    "pressure P and density RHO missing" );
            }
        }
        ...
        if (atm_profile->contains_vector( "U" )) {
            atm_profile->convert_property_units( "U",
                                                 Units::fromString( "m/s" ) );
        } else {
            throw std::runtime_error(
                "U wind vector not found in atmosphere file" );
        }
        if (atm_profile->contains_vector( "V" )) {
            atm_profile->convert_property_units( "V",
                                                 Units::fromString( "m/s" ) );
        } else {
            throw std::runtime_error(
                "V wind vector not found in atmosphere file" );
        }
        if (atm_profile->contains_vector( "T" )) {
            atm_profile->convert_property_units( "T",
                                                 Units::fromString( "K" ) );
        } else {
            throw std::runtime_error(
                "T temperature vector not found in atmosphere file" );
        }
        if (atm_profile->contains_vector( "P" )) {
            atm_profile->convert_property_units( "P",
                                                 Units::fromString( "Pa" ) );
        } else {
            throw std::runtime_error(
                "P pressure vector not found in atmosphere file" );
        }
        if (atm_profile->contains_vector( "RHO" )) {
            atm_profile->convert_property_units(
                "RHO", Units::fromString( "kg/m3" ) );
        } else {
            throw std::runtime_error(
                "RHO density vector not found in atmosphere file" );
        }
        if (atm_profile->contains_scalar( "Z0" )) {
            atm_profile->convert_property_units( "Z0",
                                                 Units::fromString( "m" ) );
            double profile_z0 = atm_profile->get( "Z0" );
            if (z_min < profile_z0) {
                std::cout << "Adjusting minimum altitude to profile ground "
                             "height of "
        ...
        if (usrattfile.empty()) {
            atm_profile->calculate_attenuation( "_ALPHA_", "T", "P", "RHO",
                                                freq );
        } else {
    ...
    int NCPA::ESSModeSolver::solve() {
        ...
        k2     = new double[ NCPAPROP_MAX_MODES ];
        ...
        SlepcInitialize( PETSC_NULLPTR, PETSC_NULLPTR, (char *)0,
                         PETSC_NULLPTR ); /* @todo move out of loop? */
        ...
        for (fi = 0; fi < Nfreq; fi++) {
            ...
            for (it = 0; it < Naz; it++) {
                ...
                cout << endl
                     << "Now processing azimuth = " << azi << " (" << it + 1
                     << " of " << Naz << ")" << endl;

                atm_profile->calculate_wind_component( "_WC_", "_WS_", "_WD_",
                                                       azi );
                atm_profile->calculate_effective_sound_speed( "_CEFF_", "_C0_",
                                                              "_WC_" );
                atm_profile->get_property_vector( "_CEFF_", c_eff );
                atm_profile->add_property(
                    "_AZ_", azi,
                ...
                if (gnd_imp_model.compare( "rigid" ) == 0) {
                    ...
                    if (Lamb_wave_BC) {
                        admittance = -atm_profile->get_first_derivative(
                                         "RHO", z_min )
                    ...
                    throw invalid_argument(
                        "This ground impedance model is not implemented yet: "
                        + gnd_imp_model );
                }
                ...
                //
                getModalTrace( Nz_grid, z_min, sourceheight, receiverheight,
                               dz, atm_profile, admittance, freq, azi, diag,
                               &k_min, &k_max, use_WKB, c_eff );
                ...
                printf(
                    ...
                    "_____________\n\n" );
                printf(
                    " -> Normal mode solution at %5.3f Hz and %5.2f deg (%d "
                    "modes)...\n",
                    freq, azi, nev );
                printf( " -> Discrete spectrum: %5.2f m/s to %5.2f m/s\n",
                        2 * PI * freq / k_max, 2 * PI * freq / k_min );

                NCPA::EigenEngine::doESSCalculation(
                    diag, Nz_grid, dz, tol, &k_min, &k_max, &nconv, k2, v );

                // select modes and do perturbation
                doSelect( Nz_grid, nconv, k_min, k_max, k2, v, k_s, v_s,
                          &select_modes );
                doPerturb( Nz_grid, z_min, dz, select_modes, freq, k_s, v_s,
                           alpha, k_pert );
                std::ofstream ofs( "modess_eigenvecs.dat",
                                   ios_base::trunc | ios_base::out );
                ofs << select_modes << " " << Nz_grid << std::endl;
                for (int ii = 0; ii < select_modes; ++ii) {
                    ofs << k_s[ ii ];
                    for (int jj = 0; jj < Nz_grid; ++jj) {
                        ofs << " " << v_s[ jj ][ ii ] << " 0.0";
                    }
                    ofs << std::endl;
                }
                ...
                if (Nby2Dprop) {  // if (N by 2D is requested)
                    fname_lossy = tag_filename(
                        NCPAPROP_MODESS_FILENAME_1D_MULTIPROP );
                    fname_lossless = "";
                    if (write_lossless) {
                        fname_lossless = tag_filename(
                            NCPAPROP_MODESS_FILENAME_1D_LOSSLESS_MULTIPROP );
                    }
                    getTLoss1DNx2( azi, select_modes, dz, Nrng_steps, rng_step,
                                   sourceheight, receiverheight, rho, k_pert,
                                   v_s, Nby2Dprop, it, fname_lossy,
                                   fname_lossless );
                } else {
                    ...
                    if (write_lossless) {
                        fname_lossless = tag_filename(
                            NCPAPROP_MODESS_FILENAME_1D_LOSSLESS );
                        std::cout << "Lossless: " << fname_lossless
                                  << std::endl;
                    }
                    getTLoss1D( select_modes, dz, Nrng_steps, rng_step,
                                sourceheight, receiverheight, rho, k_pert, v_s,
                    ...
                    if (!modstartfile.empty()) {
                        ...
                        // Modification to apply the sqrt(k0) factor to agree
                        // with Jelle's getModalStarter; this in turn will make
                        // 'pape' agree with modess output
                        getModalStarter( Nz_grid, select_modes, dz, freq,
                                         sourceheight, receiverheight, rho,
                                         k_pert, v_s,
                                         tag_filename( modstartfile ) );
                    }

                    if (write_2D_TLoss) {
                        std::cout << "Writing to file: 2D transmission loss"
                                  << std::endl;
                        fname_lossy
                            = tag_filename( NCPAPROP_MODESS_FILENAME_2D );
                        std::cout << "Lossy: " << fname_lossy << std::endl;
                        fname_lossless = "";
                        if (write_lossless) {
                            fname_lossless = tag_filename(
                                NCPAPROP_MODESS_FILENAME_2D_LOSSLESS );
                            std::cout << "Lossless: " << fname_lossless
                                      << std::endl;
                        }
                        getTLoss2D( Nz_grid, select_modes, dz, Nrng_steps,
                                    rng_step, sourceheight, rho, k_pert, v_s,
                                    fname_lossy, fname_lossless );
                    }

                    if (write_phase_speeds) {
                        std::cout << "Writing to file: phase speeds"
                                  << std::endl;
                        writePhaseSpeeds(
                            select_modes, freq, k_pert,
                            tag_filename(
                                NCPAPROP_MODESS_FILENAME_PHASE_SPEEDS ) );
                    }

                    if (write_modes) {
                        cout << "Writing to file: the modes and the phase and "
                                "group speeds...\n";
                        writeEigenFunctions( Nz_grid, select_modes, dz, v_s,
                                             "nm" );
                        writePhaseAndGroupSpeeds(
                            Nz_grid, dz, select_modes, freq, k_pert, v_s,
                            c_eff,
                            tag_filename(
                                NCPAPROP_MODESS_FILENAME_PHASE_AND_GROUP_SPEEDS ) );
                    }

                    if (( write_speeds ) && !( write_modes )) {
                        cout << "Writing to file: the modal phase speeds and "
                                "the "
                                "group speeds...\n";
                        writePhaseAndGroupSpeeds(
                            Nz_grid, dz, select_modes, freq, k_pert, v_s,
                    ...
                    if (!dispersion_file.empty()) {
                        printf(
                            "Writing to file %s: dispersion at freq = %8.3f "
                            "Hz...\n",
                            dispersion_file.c_str(), freq );
                        FILE *dispersionfile;
                        if (append_dispersion_file) {
                            dispersionfile
                                = fopen( dispersion_file.c_str(), "a" );
                        } else {
                            dispersionfile
                                = fopen( dispersion_file.c_str(), "w" );
                        }
                        writeDispersion( dispersionfile, select_modes, dz,
                                         sourceheight, receiverheight, freq,
                                         k_pert, v_s, rho );
                        fclose( dispersionfile );
                    }

                    if (write_atmosphere && ( fi == 0 )) {
                        std::string atm_output_filename = tag_filename(
                            NCPAPROP_MODESS_FILENAME_ATMOSPHERE );
                ...
                // Clean up azimuth-specific atmospheric properties before
                // starting next run
    ...
    int NCPA::ESSModeSolver::getModalTrace(
        int nz, double z_min, double sourceheight, double receiverheight,
        double dz, Atmosphere1D *p, double admittance, double freq, double azi,
        double *diag, double *k_min, double *k_max, bool use_WKB,
        double *ceffz ) {
        ...
        // double azi_rad  = NCPA::Units::convert( p->get( "_AZ_" ),
        // NCPA::UNITS_ANGLE_DEGREES, UNITS_ANGLE_RADIANS ); double gamma
        // = 1.4; double z_km = z_min_km;
        ...
        *k_min    = omega / ceffmax;
        for (i = 0; i < nz; i++) {
            ...
            // we neglect rho_factor - it does not make sense to have in this
            // approximation
            // rho_factor is 1/2*rho_0"/rho_0 - 3/4*(rho_0')^2/rho_0^2
            diag[ i ]  = pow( omega / ceffz[ i ], 2 );

            if (ceffz[ i ] < ceffmin) {
                ceffmin = ceffz[ i ];  // approximation to find minimum sound
                                       // speed in problem
            }
            if (ceffz[ i ] > ceffmax) {
                ceffmax = ceffz[ i ];
                if (i <= (int)( (double)nz
                                * NCPAPROP_MODESOLVER_K_DOMAIN_FRACTION )) {
                    *k_min = omega / ceffmax;
                }
            }
        ...
        bnd_cnd = ( 1. / ( dz * admittance + 1 ) )
                / ( pow( dz, 2 ) );  // bnd cnd assuming centered fd
        diag[ 0 ] = bnd_cnd + diag[ 0 ];

        // use WKB trick for ground to ground propagation.
        bool used_WKB = false;
        if (( fabs( sourceheight ) < 1.0e-3 )
            && ( fabs( receiverheight ) < 1.0e-3 ) && ( use_WKB )) {
            //
            ...
            //
            // dv: when ceffmin is at the ground dkk can be very small but
            // non-zero (rounding errors?) in that case we need to skip the
            // next {for loop}; otherwise it will be executed with the small
            // increment dkk
            //
            kk = pow( k_gnd,
                      2 );  // initialization of kk in case the loop is skipped
            if (dkk > 1.0e-10) {  // do this loop only if dkk is nonzero
                                  // (significantly)
                for (kk = pow( k_gnd, 2 ); kk < pow( k_max_full, 2 );
                     kk = kk + dkk) {
                    wkbIntegral = 0.0;
                    wkbTerm     = 1.0;
                    i           = 0;
                    ...
                    while (wkbTerm > dkk) {
                        ...
                        wkbIntegral
                            = wkbIntegral
                            + dz * sqrt( wkbTerm );  // dz should be in meters
                        i++;
                        // z_km += dz_km;
                    }

                    if (wkbIntegral >= 10.0) {
                        printf( "\nWKB fix: new phasevelocity minimum= %6.2f "
                                "m/s (was %6.2f m/s)\n",
            ...
            *k_max = sqrt( kk );  // use this for ground-to-ground 1D Tloss
            //(uses WKB trick to include only non-vanishing modes at the
            // ground)
            // *k_max = k_max_full;
        ...
        // double cefftop = p->get( "_CEFF_", Hgt[ top+1 ] );
        // *k_min = omega / cefftop;
    ...
    int NCPA::ESSModeSolver::doSelect( int nz, int n_modes, double k_min,
                                       double k_max, double *k2, double **v,
                                       double *k_s, double **v_s,
                                       int *select_modes ) {
        int cnt = -1;
        int i, j;
        double k;
    ...
    // Modal starter - DV 20151014
    // Modification to apply the sqrt(k0) factor to agree with Jelle's
    // getModalStarter; this in turn will make 'pape' agree with modess output
    void NCPA::ESSModeSolver::getModalStarter(
        int nz, int select_modes, double dz, double freq, double z_src,
        double z_rcv, double *rho, complex<double> *k_pert, double **v_s,
        const std::string& modstartfile ) {
        // computes the modal starter field useful to ingest in a subsequent PE
        // run. The formula for the modal starter as in Ocean Acoustics, 1994
        // ed. eq. 6.72, pg. 361: where Psi_m(z) are the modes defined in
        // (Ocean Acoustics, 1994 ed. pg. 274.) Eq. 6.72 is the "normalized"
        // modal field which in this case means that it is divided by
        // (1/(4*PI)) i.e. multiplied by (4*PI) the abs(pressure of point
        // source in free space). We do not need this normalization so we'll
        // use formula 6.72 / (4*PI)


        // Psi_starter(z) = sqrt(2*PI)/(4*PI)*sqrt(rho(z))/sqrt(rho(zs))*sum(
        // Vm(z)*Vm(zs)/sqrt(k_m) ) where Vm are the modes computed in this
        // code. Note that Vm(z) = Psi_m(z)/sqrt(rho(z)),
        //
        ...
        for (j = 0; j < nz; j = j + z_cnd) {
            ...
            for (m = 0; m < select_modes; m++) {
                modal_sum = modal_sum
                          + v_s[ n_zsrc ][ m ] * v_s[ j ][ m ]
                                / sqrt( real( k_pert[ m ] ) );
            }

            // modal_sum = factor/twoPI*sqrtTwo*sqrtrhoj*modal_sum*sqrt(k0); //
            // modal starter at particular z

            modal_sum
                = modal_sum * PI * sqrt( k0 );  // Jelle - needs the sqrt(k0)
            // modal_sum = factor/twoPI*sqrtTwo*sqrtrhoj*modal_sum; // modal
            // starter at particular z

            // for (m=0; m<select_modes; m++) {
            //     modal_sum = modal_sum +
            //     v_s[n_zsrc][m]*v_s[j][m]/sqrt(k_pert[m]);
            // }
            // modal_sum = sqrt2PI/rhos*modal_sum; // modal starter at
            // particular z modal_sum = factor/rhos*modal_sum; // modal starter
            // at particular z
    ...
    int NCPA::ESSModeSolver::writePhaseAndGroupSpeeds(
        int nz, double dz, int select_modes, double freq,
        ...
        for (j = 0; j < select_modes; j++) {
            ...
            // compute the group speed: vg = v_phase*int_0^z_max
            // Psi_j^2/c_eff^2 dz_km
            v_group = 0.0;
            for (n = 0; n < nz; n++) {
                v_group = v_group
                        + v_s[ n ][ j ] * v_s[ n ][ j ]
