...
#  define NCPAPROP_MODESS_FILENAME_1D_LOSSLESS_MULTIPROP \
...
namespace NCPA {
    ...
    class ESSModeSolver : public ModeSolver {
        public:
            ...
            int solve();
            // int doESSSLEPcCalculation( double *diag, double dz, double
            // *k_min, double *k_max, 	PetscInt *nconv, double *k2, double **v
            //);
            int doSelect( int nz, int n_modes, double k_min, double k_max,
                          double *k2, double **v, double *k_s, double **v_s,
                          int *select_modes );
            int getModalTrace( int nz, double z_min, double sourceheight,
                               double receiverheight, double dz,
                               NCPA::Atmosphere1D *p, double admittance,
                               double freq, double azi, double *diag,
                               double *k_min, double *k_max, bool use_WKB,
                               double *c_eff );
            void getModalStarter( int nz, int select_modes, double dz,
                                  double freq, double z_src, double z_rcv,
                                  double *rho, std::complex<double> *k_pert,
                                  double **v_s,
                                  const std::string& modstartfile );
            int writePhaseAndGroupSpeeds( int nz, double dz, int select_modes,
                                          double freq,
                                          std::complex<double> *k_pert,
                                          double **v_s, double *c_eff,
