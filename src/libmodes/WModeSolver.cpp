...
NCPA::WModeSolver::WModeSolver( NCPA::ParameterSet *param,
...
void NCPA::WModeSolver::setParams( NCPA::ParameterSet *param,
...
int NCPA::WModeSolver::solve() {
    ...
    for (fi = 0; fi < Nfreq; fi++) {
        ...
        for (it = 0; it < Naz; it++) {
            ...
            // compute absorption
            // getAbsorption(Nz_grid, dz, atm_profile, freq, usrattfile,
            // alpha);
            ...
            //Lamb_wave_BC admittance =
            -atm_profile->drhodz(z_min/1000.0)/1000.0/atm_profile->rho(z_min/1000.0)/2.0;
            // SI units
            }
            ...
            gnd_imp_model; throw invalid_argument(es.str());
            }
            */
            admittance = 0.0;  // default
            ...
            getModalTrace( Nz_grid, z_min, sourceheight, receiverheight, dz,
                           atm_profile, admittance, freq, diag, kd, md, cd,
                           &k_min, &k_max, use_WKB );
            ...
            //
            // set up parameters for eigenvalue estimation; double dimension of
            // problem for linearization
            //
            std::ostringstream oss;
            oss << "__________________________________________________________"
                   "____________"
                << std::endl
                << std::endl
                << " -> Solving wide-angle problem at " << freq << " Hz and "
                << azi << "deg (" << nev * 2 << " modes)..." << std::endl
                << " -> Discrete spectrum: " << 2 * PI * freq / k_max << " to "
                << 2 * PI * freq / k_min << " m/s" << std::endl
                << " -> Quadratic eigenvalue problem  - double "
                   "dimensionality.";

            NCPA::EigenEngine::doWideAngleCalculation(
                Nz_grid, dz, k_min, k_max, tol, nev, kd, md, cd, &nconv, kH, v,
                oss.str() );

            // select modes and do perturbation
            doSelect( Nz_grid, nconv, k_min, k_max, kH, v, k_s, v_s,
                      &select_modes );
            ...
            if (Nby2Dprop) {  // if (N by 2D is requested)
                fname_lossy
                    = tag_filename( NCPAPROP_WMOD_FILENAME_1D_MULTIPROP );
                fname_lossless = "";
                if (write_lossless) {
                    fname_lossless = tag_filename(
                        NCPAPROP_WMOD_FILENAME_1D_LOSSLESS_MULTIPROP );
                }
                ...
                if (write_lossless) {
                    fname_lossless
                        = tag_filename( NCPAPROP_WMOD_FILENAME_1D_LOSSLESS );
                ...
                if (write_2D_TLoss) {
                    ...
                    if (write_lossless) {
                        fname_lossless = tag_filename(
                            NCPAPROP_WMOD_FILENAME_2D_LOSSLESS );
...
int NCPA::WModeSolver::doSelect( int nz, int n_modes, double k_min,
                                 double k_max, double *kH, double **v,
...
int NCPA::WModeSolver::getModalTrace( int nz, double z_min,
                                      double sourceheight,
                                      double receiverheight, double dz,
                                      Atmosphere1D *p, double admittance,
                                      double freq, double *diag, double *kd,
                                      double *md, double *cd, double *k_min,
    ...
    // azi_rad  = NCPA::Units::convert( p->get( "_AZ_" ),
    // NCPA::UNITS_ANGLE_DEGREES, UNITS_ANGLE_RADIANS );
    ...
    for (i = 0; i < nz; i++) {
        ...
        // rho_factor = (-3/4*pow(p->drhodz(z_km)/p->rho(z_km),2) +
        // 1/2*p->ddrhodzdz(z_km)/p->rho(z_km))*1e-6; // in meters^(-2)!
        ...
        if (ceffz[ i ] < ceffmin)
            ceffmin = ceffz[ i ];  // approximation to find minimum sound speed
                                   // in problem
        if (ceffz[ i ] > ceffmax) ceffmax = ceffz[ i ];
    ...
    if (( fabs( sourceheight ) < 1.0e-3 )
        ...
        //
        // dv: when ceffmin is at the ground dkk can be very small but non-zero
        // (rounding errors?) in that case we need to skip the next for loop;
        // otherwise it will be executed with the small increment dkk
        //
        kk = pow( k_gnd,
                  2 );  // initialization of kk in case the loop is skipped

        if (dkk
            > 1e-10) {  // do this loop only if dkk is nonzero (significantly)
            for (kk = pow( k_gnd, 2 ); kk < pow( k_max_full, 2 );
                ...
                if (wkbIntegral >= 10.0) {
                    printf( "\n WKB fix: new phasevelocity minimum: %6.2f m/s "
                            "(was %6.2f m/s); \n WKBIntegral= %12.7f at z = "
                            "%6.2f km\n",
