...
protected:
	...
	void SetUp() override {
		new_v5 = BasicVector<int>( 5, {0,1,4}, {1,2,5} );

		smat1 = new BasicMatrix<int>( 5, 8 );
		...
		smat2_t = new BasicMatrix<int>( 5, 8 );
		...
		smat3 = new BasicMatrix<int>( *smat2 );
	}
	...
	// class members here
	BasicMatrix<int> *smat1, *smat2, *smat2_t, *smat3, *smat4, smat5;
	Matrix<int> *mat1;
	BasicVector<int> new_v5, new_v8;
};

TEST_F(BasicMatrixTest,GetColumnIndicesWorksCorrectly) {
...
TEST_F(BasicMatrixTest,EqualityOperatorReturnsTrueIfEqual) {
	EXPECT_TRUE( *smat1 == *smat1 );
}

TEST_F(BasicMatrixTest,EqualityOperatorReturnsFalseIfNotEqual) {
	EXPECT_FALSE( *smat1 == *smat2 );
}

TEST_F(BasicMatrixTest,InequalityOperatorReturnsFalseIfEqual) {
	EXPECT_FALSE( *smat1 != *smat1 );
}

TEST_F(BasicMatrixTest,InequalityOperatorReturnsTrueIfNotEqual) {
	EXPECT_TRUE( *smat1 != *smat2 );
}

TEST_F(BasicMatrixTest,CopyConstructorCreatesCorrectCopy) {
	EXPECT_TRUE( *smat2 == *smat3 );
}

TEST_F(BasicMatrixTest,AssignmentOperatorCreatesCopy) {
	smat5 = *smat2;
	EXPECT_TRUE( smat5 == *smat2 );
	EXPECT_FALSE( &smat5 == smat2 );
}

TEST_F(BasicMatrixTest,CloneMethodCreatesCopy) {
	mat1 = smat2->clone();
...
TEST_F(BasicMatrixTest,MatricesHaveCorrectRowCounts) {
	ASSERT_EQ( smat1->rows(), 5 );
	ASSERT_EQ( smat2->rows(), 8 );
}

TEST_F(BasicMatrixTest,MatricesHaveCorrectColumnCounts) {
	ASSERT_EQ( smat1->columns(), 8 );
	ASSERT_EQ( smat2->columns(), 5 );
}

TEST_F(BasicMatrixTest,InitializeSetsRowsCorrectly) {
	smat1->initialize( 3, 3 );
	EXPECT_EQ( smat1->rows(), 3 );
}

TEST_F(BasicMatrixTest,InitializeSetsColumnsCorrectly) {
	smat1->initialize( 3, 3 );
	EXPECT_EQ( smat1->columns(), 3 );
}

TEST_F(BasicMatrixTest,GetRetrievesCorrectValues) {
	EXPECT_EQ( smat1->get( 2, 5 ), 1 );
	EXPECT_EQ( smat2->get( 0, 3 ), 2 );
}

TEST_F(BasicMatrixTest,SetAltersValuesCorrectly) {
	ASSERT_EQ( smat1->get( 2, 5 ), 1 );
	smat1->set( 2, 5, 10 );
	ASSERT_EQ( smat1->get( 2, 5 ), 10 );
}

TEST_F(BasicMatrixTest,GetRowGetsRowCorrectlyAsArray) {
	int *row = zeros<int>( 8 );
	size_t n = 8;
...
TEST_F(BasicMatrixTest,GetRowCreatesArrayIfPassedNullptr) {
	int *row = nullptr;
	size_t n = 0;
	int expected[8] = { 0, 2, 3, 3, 3, 1, 1, 1 };
	smat1->get_row( 2, n, row );
...
TEST_F(BasicMatrixTest,GetRowThrowsOutOfRangeIfWrongSizeArrayProvided) {
	int *row = zeros<int>( 6 );
	size_t n = 6;
	EXPECT_THROW( {smat1->get_row( 2, n, row );}, out_of_range );
}

TEST_F(BasicMatrixTest,GetRowGetsRowCorrectlyAsVectorArgument) {
	vector<int> row;
	int expected[8] = { 0, 2, 3, 3, 3, 1, 1, 1 };
	smat1->get_row( 2, row );
...
TEST_F(BasicMatrixTest,GetRowReturnsCorrectVector) {
	const NCPA::Vector<int> *v = smat1->get_row( 2 );
	EXPECT_EQ( v->size(), 8 );
	int expected[8] = { 0, 2, 3, 3, 3, 1, 1, 1 };
...
TEST_F(BasicMatrixTest,GetColumnGetsColumnCorrectlyAsArray) {
	int *col = zeros<int>( 5 );
	size_t n = 5;
...
TEST_F(BasicMatrixTest,GetColumnCreatesArrayIfPassedNullptr) {
	int *col = nullptr;
	size_t n = 0;
	int expected[5] = { 3, 3, 0, 3, 2 };
	smat1->get_column( 0, n, col );
...
TEST_F(BasicMatrixTest,GetColumnThrowsOutOfRangeIfWrongSizeArrayProvided) {
	int *col = zeros<int>( 6 );
	size_t n = 6;
	EXPECT_THROW( {smat1->get_column( 2, n, col );}, out_of_range );
}

TEST_F(BasicMatrixTest,GetColumnGetsColumnCorrectlyAsVectorArgument) {
	vector<int> col;
	int expected[5] = { 3, 3, 0, 3, 2 };
...
TEST_F(BasicMatrixTest,GetColumnReturnsCorrectVector) {
	const NCPA::Vector<int> *v = smat1->get_column( 0 );
	EXPECT_EQ( v->size(), 5 );
	int expected[5] = { 3, 3, 0, 3, 2 };
	for (size_t i = 0; i < 5; i++) {
		EXPECT_EQ( v->get(i), expected[i] );
	}
//	EXPECT_ARRAY_EQ( 5, (*v), expected );
}

TEST_F(BasicMatrixTest,SetRowWithArraySetsRowCorrectly) {
...
TEST_F(BasicMatrixTest,SetRowReturnsThis) {
	size_t *inds = NCPA::zeros<size_t>( 5 );
	int *vals = NCPA::zeros<int>( 5 );
	for (size_t i = 0; i < 5; i++) {
		inds[i] = i;
	...
	NCPA::BasicMatrix<int> *ptr = static_cast<NCPA::BasicMatrix<int>*>(
			smat2->set_row( 0, 5, inds, vals ) );
	EXPECT_EQ(ptr,smat2);
}

TEST_F(BasicMatrixTest,SetRowWithVectorSetsRowCorrectly) {
	vector<int> vals(5);
	for (size_t i = 0; i < 5; i++) {
		vals[i] = i+1;
	}
...
TEST_F(BasicMatrixTest,SetRowWithTwoVectorsSetsRowCorrectly) {
...
TEST_F(BasicMatrixTest,SetRowWithBasicVectorSetsRowCorrectly) {
	smat2->set_row( 0, &new_v5 );
	vector<int> row0;
	smat2->get_row( 0, row0 );
...
TEST_F(BasicMatrixTest,SetColumnWithArraySetsColumnCorrectly) {
	size_t *inds = NCPA::zeros<size_t>( 5 );
	int *vals = NCPA::zeros<int>( 5 );
	for (size_t i = 0; i < 5; i++) {
		inds[i] = i;
...
TEST_F(BasicMatrixTest,SetColumnReturnsThis) {
	size_t *inds = NCPA::zeros<size_t>( 5 );
	int *vals = NCPA::zeros<int>( 5 );
	for (size_t i = 0; i < 5; i++) {
		inds[i] = i;
...
TEST_F(BasicMatrixTest,SetColumnWithVectorSetsColumnCorrectly) {
	vector<int> vals(5);
	for (size_t i = 0; i < 5; i++) {
		vals[i] = i+1;
	}
...
TEST_F(BasicMatrixTest,SetColumnWithTwoVectorsSetsColumnCorrectly) {
	vector<int> vals = { 1, 2, 5 };
	vector<size_t> inds = {0, 1, 3 };
...
TEST_F(BasicMatrixTest,SetColumnWithBasicVectorSetsColumnCorrectly) {
	smat1->set_column( 0, &new_v5 );
	vector<int> col0;
	smat1->get_column( 0, col0 );
	vector<int> expected = { 1, 2, 0, 0, 5 };
	EXPECT_ARRAY_EQ( 5, col0, expected );
...
TEST_F(BasicMatrixTest,IsReadyReturnsTrueForPopulatedMatrix) {
	EXPECT_TRUE( smat1->is_ready() );
}

TEST_F(BasicMatrixTest,IsReadyReturnsFalseForUnpopulatedMatrix) {
	EXPECT_FALSE( smat5.is_ready() );
}

TEST_F(BasicMatrixTest,AdditionWorksCorrectly) {
	smat2->add( smat3 );
	BasicVector<int> v;
...
TEST_F(BasicMatrixTest,AdditionThrowsOutOfRangeIfSizesDoNotMatch) {
	EXPECT_THROW( { smat2->add( smat1 ); }, out_of_range );
}

TEST_F(BasicMatrixTest,SubtractionWorksCorrectly) {
	smat2->subtract( smat3 );
	BasicVector<int> *v = new BasicVector<int>();
	for (size_t row = 0; row < smat2->rows(); row++) {
		smat2->get_row(row, v);
		for (auto it = v->begin(); it != v->end(); ++it) {
...
TEST_F(BasicMatrixTest,SubtractionThrowsOutOfRangeIfSizesDoNotMatch) {
	EXPECT_THROW( { smat2->subtract( smat1 ); }, out_of_range );
}

TEST_F(BasicMatrixTest,TransposeWorksCorrectly) {
...
TEST_F(BasicMatrixTest,ScaleWorksCorrectly) {
	smat3->add(smat2)->add(smat2);
	smat2->scale(3.0);
	EXPECT_TRUE( *smat2 == *smat3 );
}

TEST_F(BasicMatrixTest,ScaleWorksCorrectlyForNegativeScalars) {
...
//TEST(BasicMatrixTest,TestName2) {
//
//}
