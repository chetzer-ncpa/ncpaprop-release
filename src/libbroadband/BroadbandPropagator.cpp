...
// 	single_receiver 		= param->getString("receiver").compare("single") ==
// 0;
...
// 		if (rmax <= rmin) {
// 			throw std::runtime_error( "end_range_km must be greater than
// start_range_km" );
// 		}
// 		Nr = (int)floor( (rmax-rmin) / rstep ) + 1;
// 		r_vec = new double[ Nr ];
// 		for (int i = 0; i < Nr; i++) {
// 			r_vec[ i ] = (rmin + ((double)i) * rstep) * 1000.0;  // expected in
// meters
// 		}
// 	}

// 	Nfreq = NCPA::count_rows_arbcol( dispersion_input_file ) + 1;    // count
// DC frequency 	f_vec = new double[ Nfreq ]; 	mode_count = new int[ Nfreq ];
...
//   // reads freq #modes rho(@z=src), rho(@z=rcv), k_horiz, modes(@z=src),
//   modes(@z=receiverheight) int m,n; double x1,x2,y1,y2; FILE *fp =
//   fopen(dispersion_input_file.c_str(),"r"); if(fp==NULL){
//       std::ostringstream es;
//       es << "file << " << dispersion_input_file.c_str() << " could not be
//       opened.\n"; throw std::invalid_argument(es.str());
//   }
//   //read_header(f); // use it if file has header
//   printf("--> Opening dispersion file %s ...if the program chokes here the
//   file may be corrupt.\n",
...
//               fprintf(stderr,"%s: read_dispersion_data format
//               error\n",dispersion_input_file.c_str()); exit(1);
//           }
//           re_k[n][m]  =x1;
//           im_k[n][m]  =x2;
//           mode_S[n][m]=y1;
...
//   // if zero attenuation requested set the imaginary part of the wavenumber
//   = 0 if (zero_attn_flag) {
//     for (n=0; n<Nfreq; n++) {
...
// 	//     std::cout << "--> Doing pulse propagation source-to-receiver at one
// range: "
...
// 	//     //
// 	//     // fft propagation: note that here 'pulse_vec' is overwritten with
// the propagated pulse
// 	//     //
...
//  //      // DV 20170810 - parameter factor to make it easy to agree with
//  other codes
...
//  //      std::cout << "--> Propagated pulse saved in file: '" <<
//  waveform_out_file << "'" << std::endl
...
// 	  std::cout << "--> Propagating pulse from source-to-receivers on grid ..."
// << std::endl;
//       printf("----------------------------------------------\n");
...
//       waveform_out_file.c_str()); printf("with columns: R (km) | time (s) |
//       pulse(R,t) |\n");
//   // }
...
int NCPA::BroadbandPropagator::get_source_spectrum(
    ...
    if ( source_type.compare( "impulse" )
        ...
        // ----------- do pulse choice # 2 ---------------------
            ...
            model_pulse_fft(
                power, scale, dt,
                arg_vec );  // arg_vec is the FT of initial pulse choice #2
            for ( i = 0; i < Nfreq; i++ ) {
                dft_vec[ i ]
                    = arg_vec[ i ];  //  multiplication by the dt factor
        ...
        if ( fabs( freqv[ 0 ] - f_vec[ 0 ] )
             > 1.0e-8 ) {  // compare frequencies
...
void NCPA::BroadbandPropagator::model_pulse_fft(
    double power, double scale, double dt, std::complex<double> *dft_vec ) {
    int i;
    ...
    for ( i = 0; i < NFFT; i++ ) {
        p_vec[ i ] = model_pulse_shape( power, scale,
                                        i * dt );  // the pulse timeseries
    }
...
//       }
//       // Note on mode scaling in this code vs. the modes in Computational
//       Oc. Acoust. 1994:
//       // V_book =  sqrt(rho)*V_in_this_code;
//       // Thus the formula for the Fourier component of pressure using the
//       modes in this code
...
//   smooth_space=(int)floor(0.1*Nfreq); // smoothly zero out on right; as in
//   RW (July 2012)

//   for(i=Nfreq-smooth_space;i<Nfreq;i++){
//       modal_sum[i]=modal_sum[i]*half_hann(Nfreq-smooth_space,Nfreq-1,i); //
//       changed df to 1 as df doesn't make sense here
//   }
// }
...
void NCPA::BroadbandPropagator::fft_pulse_prop(
    ...
    for ( i = 0; i < Nfreq; i++ ) {
        ...
        t_phase = exp(
            -I * 2.0 * Pi * f_vec[ i ]
            * t0 );  // corresponding to reduced time t0; note f_vec[i+1]
        // cup=0.0;
        // for(j=0;j<mode_count[i];j++){
        //     k_H = re_k[i][j]+I*im_k[i][j];
        //     pot = exp(I*range*k_H);
        ...
        // up to a factor ((delayed Fourier pressure
        // component)*source_spectrum*df) (see eq. 5.14 pg 274 and eq. 8.9 pg
        // 480 in Comp. Oc. Acoust. 1994 ed.)
        // cup=cup*dft_vec[i]*df;
        // cup=cup*dft_vec[i]; // will multiply by df at the end; note
        // f_vec[i+1]
    ...
    //     // old code : has df in it
    //     arg_vec[i]=arg_vec[i]*half_hann(Nfreq-smooth_space,Nfreq-1,i); //
    //     changed df to 1 as df doesn't make sense here
    // }
