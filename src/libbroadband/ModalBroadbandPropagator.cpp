...
NCPA::ModalBroadbandPropagator::ModalBroadbandPropagator(
    ...
    if (single_receiver) {
        ...
        r_vec.resize( 1, param->getFloat( "range_km" ) * 1000.0 );
    } else {
        ...
        if (rmax <= rmin) {
            throw std::runtime_error(
                "end_range_km must be greater than start_range_km" );
        }
        Nr = (int)floor( ( rmax - rmin ) / rstep ) + 1;
        // r_vec = new double[ Nr ];
        r_vec.resize( Nr, 0.0 );
        for (int i = 0; i < Nr; i++) {
            r_vec[ i ] = ( rmin + ( (double)i ) * rstep )
                       * 1000.0;  // expected in meters
        }
    }

    Nfreq = NCPA::count_rows_arbcol( dispersion_input_file )
          + 1;  // count DC frequency
    // f_vec      = new double[ Nfreq ];
    f_vec.clear();
    f_vec.resize( Nfreq, 0.0 );
    // mode_count = new int[ Nfreq ];
    mode_count.clear();
    mode_count.resize( Nfreq, 0 );
    ...
    // if you're given a source file, make sure you can fit the whole thing in
    // NFFT points
    if (source_type.compare( "waveform" ) == 0) {
        int filelines = NCPA::count_rows_arbcol( source_file.c_str() );
        while (filelines > NFFT) {
            NFFT *= 2;
            std::cout << "File " << source_file << " has " << filelines
                      << " lines, increasing NFFT to " << NFFT << std::endl;
        }
    }
}
...
int NCPA::ModalBroadbandPropagator::read_dispersion_file() {
    ...
    // modes(@z=receiverheight)
    int m, n;
    double x1, x2, y1, y2;
    FILE *fp = fopen( dispersion_input_file.c_str(), "r" );
    if (fp == NULL) {
        std::ostringstream es;
        es << "file << " << dispersion_input_file.c_str()
           << " could not be opened.\n";
...
int NCPA::ModalBroadbandPropagator::calculate_waveform() {
    ...
    transfer_function.resize( Nfreq, 0.0 );
    ...
    for (n = 0; n < Nr; n++) {
        ...
        // std::memset( transfer_function,  0, Nfreq * sizeof( std::complex<
        // double > ) );
        // std::fill( transfer_function, transfer_function + Nfreq,
        //            std::complex<double> {} );
        std::fill( transfer_function.begin(), transfer_function.end(),
                   std::complex<double> {} );
        compute_modal_sum( rr );
        std::cout << "The first two elements in f_vec: " << f_vec[ 0 ] << ", "
                  << f_vec[ 1 ] << std::endl;
        // the call with NFFT as argument
        fft_pulse_prop( t0, rr, dft_vec, pulse_vec );

        // DV 20170810 - parameter 'factor' to make it easy to agree with other
        // codes (e.g. Roger Waxler's modal code)
        double factor = 2.0;
        for (i = 0; i < NFFT; i++) {
            fprintf( f, "%10.3f %12.6f %15.6e %15.6e\n", rr / 1000.0,
                     1.0 * i / fmx + t0, factor * real( pulse_vec[ i ] ),
                     factor * imag( pulse_vec[ i ] ) );
        }
        fprintf( f, "\n" );
    }
    fclose( f );
    ...
    for (i = 0; i < Nfreq; i++) {
        fprintf( transf_file, "%5.6f %15.6e %15.6e\n", f_vec[ i ],
                 real( transfer_function[ i ] ),
                 imag( transfer_function[ i ] ) );
    }
    fclose( transf_file );
...
void NCPA::ModalBroadbandPropagator::compute_modal_sum( double range ) {
    ...
    std::complex<double> expov8pir
        = I * exp( -I * Pi * 0.25 ) / sqrt( 8.0 * Pi * range );
    ...
    for (i = 0; i < Nfreq; i++) {
        ...
        // eq. 5.14 pg 274.
        transfer_function[ i ]
            = expov8pir * cup * sqrt_rho_ratio;  // note sqrt_rho_ratio
        // arg_vec[i0+i]=sqrt(I/(8.0*Pi*range))*cup;
    }

    // smoothly zero out on right; as in RW (July 2012)
    smooth_space = (int)floor( 0.1 * Nfreq );

    for (i = Nfreq - smooth_space; i < Nfreq; i++) {
        transfer_function[ i ]
            = transfer_function[ i ]
            * half_hann( Nfreq - smooth_space, Nfreq - 1,
                         i );  // changed df to 1 as df doesn't make sense here
    }
}
